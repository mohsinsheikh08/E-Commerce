...
const userLogout = async (req, res) => {
    try {
        ...
        return res.status(409).json({ message: "User is not logout.Try agian later!", Error: err.message })
    }
}
...
const adminLogin = async (req, res) => {
    try {
        ...
        if (!admin) {
            return res.status(409).json({ message: "Admin is not exist with this email or password!" })
        }
...
const AdminInfo = async (req, res) => {
    try {
        ...
        if (!token) {
            return res.status(200).json({ message: "No admin logged in", admin: null })
        }

        const decoded = jwt.verify(token, process.env.JWT_KEY);

        if (decoded.role !== "Admin") {
            return res.status(200).json({ message: "Not an admin", admin: null })
        }

        const admin = await adminModel.findById(decoded.id).select('-password');
        if (!admin) {
            return res.status(200).json({ message: "Admin not found", admin: null })
        }

        res.status(200).json({ message: "Admin info fetched successfully!", admin: admin })
    } catch (err) {
        return res.status(200).json({ message: "Something is wrong!", admin: null })
    }
}
...
const getStats = async (req, res) => {
    try {
        ...
        if (decoded.role !== "Admin") {
            return res.status(409).json({ message: "Admin is not available!" })
        }
...
const getAllOrders = async (req, res) => {
    try {
        const token = req.cookies.token;
        if (!token) return res.status(409).json({ message: "Invalid credintials!" })

        const decoded = jwt.verify(token, process.env.JWT_KEY);
        if (decoded.role !== "Admin") return res.status(409).json({ message: "Admin is not available!" })

        const userId = decoded.id;
        const admin = await adminModel.findById(userId).select('-password');

        if (admin.role !== "super_admin" && admin.role !== "order_manager") {
            return res.status(409).json({ message: "This page can accesible only for order_manager and super_admin!" })
        }

        const totalOrders = await orderModel.countDocuments();
        const order = await orderModel.find().populate('user', 'fullName email').sort({ createdAt: -1 });

        res.status(200).json({ message: "Orders fetched successfully!", totalOrders: totalOrders, order: order })
    } catch (err) {
        return res.status(409).json({ message: "Something is wrong!", Error: err.message })
    }
}
...
const getAllProducts = async (req, res) => {
    try {
        ...
        if (admin.role !== "super_admin" && admin.role !== "product_admin") {
            return res.status(409).json({ message: "This page is accessible only for super_admin and product_admin!" })
        }
...
const getAllUsers = async (req, res) => {
    try {
        ...
        if (admin.role !== "super_admin" && admin.role !== "user_manager") {
            return res.status(409).json({ message: "This page is accessible only for super_admin and user_manager!" })
        }
